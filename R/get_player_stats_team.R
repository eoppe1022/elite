...
get_player_stats_team <- function(..., progress = TRUE, other = "") {
  ...
  .get_player_stats_team <- function(team_url, team, league, season, ...) {
    ...
    player_stats <- page %>%
      ...
      as_tibble() %>%
      filter(row_number() > 1) %>%
      filter(row_number() <= if_else(!any(is.na(number) & name %in% c("Champions HL", "M-Cup", "Swiss Cup", "Super Series", "ET")), n(), which(is.na(number) & name %in% c("Champions HL", "M-Cup", "Swiss Cup", "Super Series", "ET"))[1])) %>%
